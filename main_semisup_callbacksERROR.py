...
import sys
# ────────────────────────────── IMPORTANTE ──────────────────────────────
# Debe ponerse ANTES de importar segmentation_models:
...
import segmentation_models as sm    # <-- Usa tf.keras gracias a SM_FRAMEWORK
...
def get_preprocessing(preprocessing_fn):
    return A.Compose([
        A.Lambda(image=preprocessing_fn, mask=None)
    ])
...
if __name__ == '__main__':
    ...
    # ───── LÓGICA “SEMI” (Mean Teacher para Cxx) ─────
        ...
        # El nombre del archivo contendrá mode y regime
        excel_name = f"{mode}_{regime}_all_metrics.xlsx"
        df_all     = pd.DataFrame(all_metrics)
        df_all.to_excel(os.path.join(excel_dir, excel_name), index=False)
        logging.info(f"✅ Métricas semi-supervisado volcadas en callbacks/{excel_name}")                        
