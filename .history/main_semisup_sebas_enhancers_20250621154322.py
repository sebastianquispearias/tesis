...
# ── Construcción del banco de kernels morfológicos ──
stats = json.load(open("data/kernel_stats.json"))   # {'radii': [21,36,53]}

def disk_kernel(r):
    k = 2*r + 1
    Y, X = tf.meshgrid(tf.range(k), tf.range(k))
    m = tf.cast((X-r)**2 + (Y-r)**2 <= r**2, tf.float32)
    # ahora devolvemos 3-D (filter_h, filter_w, in_channels)
    return tf.reshape(m, (k, k, 1))

KERNEL_BANK = [disk_kernel(r) for r in stats["radii"]]

# ── Métrica adicional para bordes (Boundary F1) ──
def bf_score(y_true, y_pred):
    return tfa.metrics.BinaryFbetaScore(beta=1.0, threshold=0.5)(y_true, y_pred)
...
# Parámetros fijos (no hace falta argumentos en CLI)
TAU = 0.90
LAMBDA_MORPH = 0.15
                    ...
                    for x_lab, y_lab, x_unl_s, x_unl_t in train_mixed_loader:
                        ...
                        with tf.GradientTape() as tape:
                           ...
                           #loss_total = loss_sup + UNLABELED_WEIGHT * cons_w * loss_cons
                            # 1) Pseudo-máscara de alta confianza
                            mask_conf = tf.cast(t_probs > TAU, tf.float32)

                            # 2) Perturbación morfológica realista (selección Python)
                            kernel = random.choice(KERNEL_BANK)
                            pert = tf.cond(
                                ...
                                lambda: tf.nn.erosion2d(mask_conf, kernel,
                                                        strides=[1,1,1,1],
                                                        dilations=[1,1,1,1],
                                                        padding='SAME',
                                                        data_format='NHWC')
                            ...
                            # 3) Straight-through trick para permitir gradientes
                            pert = tf.stop_gradient(pert) + (mask_conf - tf.stop_gradient(mask_conf))

                            # 4) Cálculo de la pérdida morfológica
                            loss_morph = tf.reduce_mean(tf.square(mask_conf - pert))

                            # 5) Pérdida total con Mean Teacher + Morph-Consistency
                            loss_total = (
                                loss_sup
                                + UNLABELED_WEIGHT * cons_w * loss_cons
                                + LAMBDA_MORPH * loss_morph
                            )
